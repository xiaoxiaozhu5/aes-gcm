    ...
    const struct ubiq_platform_algorithm ** const algo)
{
    ...
    for (unsigned int i = 0; i < ubiq_platform_algorithms_n; i++) {
        if (_stricmp(ubiq_platform_algorithms[i].name, name) == 0) {
            *algo = &ubiq_platform_algorithms[i];
            err = 0;
            break;
        }
    ...
    struct ubiq_support_hash_context ** const _ctx)
{
    ...
    const struct windigest * digest = NULL;

    struct ubiq_support_hash_context * ctx = NULL;
    int err;
    ...
    for (unsigned int i = 0;
         ...
         i++ ) {
        if (_stricmp(name, digests[i].name) == 0) {
            digest = &digests[i];
            err = 0;
            break;
        }
    ...
    struct ubiq_support_cipher_context ** const _ctx)
{
    ...
    const struct wincipher * cipher = NULL;

    struct ubiq_support_cipher_context * ctx = NULL;
    DWORD out;
    int err;
    ...
    if (!err) {
        ...
        for (unsigned int i = 0;
             ...
             i++) {
            if (_stricmp(alg->name, ciphers[i].name) == 0) {
                cipher = &ciphers[i];
                err = 0;
                break;
            }
